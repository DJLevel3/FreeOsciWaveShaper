...
def  main():
  ...
  installer = "\installer\FOWS Installer.exe"
   
  if demo:
    installer = "\installer\FOWS Demo Installer.exe"
   
  FILES_TO_ZIP = [
    projectpath + installer,
    projectpath + "\installer\changelog.txt",
    projectpath + "\installer\known-issues.txt",
    projectpath + "\manual\FOWS manual.pdf" 
  ]
  ...
  ZIPNAME = "FOWS-v" + FULLVERSIONSTR + "-win.zip"
  
  if demo:
    ZIPNAME = "FOWS-v" + FULLVERSIONSTR + "-win-demo.zip"
