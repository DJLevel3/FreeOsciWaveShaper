...
if %1 == 1 (echo Making FOWS Windows DEMO VERSION distribution ...) else (echo Making FOWS Windows FULL VERSION distribution ...)
...
REM - msbuild FOWS-app.vcxproj /p:configuration=release /p:platform=win32

echo Building 32 bit binaries...
msbuild FOWS.sln /p:configuration=release /p:platform=win32 /nologo /verbosity:minimal /fileLogger /m /flp:logfile=build-win.log;errorsonly 

echo Building 64 bit binaries...
msbuild FOWS.sln /p:configuration=release /p:platform=x64 /nologo /verbosity:minimal /fileLogger /m /flp:logfile=build-win.log;errorsonly;append
...
REM --wraptool sign --verbose --account XXXXX --wcguid XXXXX --keyfile XXXXX.p12 --keypassword XXXXX --in .\build-win\aax\bin\FOWS.aaxplugin\Contents\Win32\FOWS.aaxplugin --out .\build-win\aax\bin\FOWS.aaxplugin\Contents\Win32\FOWS.aaxplugin
REM --wraptool sign --verbose --account XXXXX --wcguid XXXXX --keyfile XXXXX.p12 --keypassword XXXXX --in .\build-win\aax\bin\FOWS.aaxplugin\Contents\x64\FOWS.aaxplugin --out .\build-win\aax\bin\FOWS.aaxplugin\Contents\x64\FOWS.aaxplugin
...
:32-Bit-is
"%ProgramFiles%\Inno Setup 5\iscc" /Q /cc ".\installer\FOWS.iss"
goto END-is

:64-Bit-is
"%ProgramFiles(x86)%\Inno Setup 5\iscc" /Q /cc ".\installer\FOWS.iss"
goto END-is

:END-is

REM - Codesign Installer for Windows 8+
REM -"C:\Program Files (x86)\Microsoft SDKs\Windows\v7.1A\Bin\signtool.exe" sign /f "XXXXX.p12" /p XXXXX /d "FOWS Installer" ".\installer\FOWS Installer.exe"

REM -if %1 == 1 (
REM -copy ".\installer\FOWS Installer.exe" ".\installer\FOWS Demo Installer.exe"
REM -del ".\installer\FOWS Installer.exe"
REM -)
